...
pub fn convert(number: &[u32], from_base: u32, to_base: u32) -> Result<Vec<u32>, Error> {
    ...
    // the above for timeout

    let len:u32 = number.len() as u32;
    let mut num:u32 = 0;
    //[4, 2]
    for (idx, &i) in number.iter().enumerate(){
        if i >= from_base { return Err(Error::InvalidDigit(i)); }
        num += i*from_base.pow(len-idx as u32 -1 );
    }
    if num == 0 { return Ok(vec![0]); }
